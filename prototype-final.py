...
class MyClass:
    ...
    def find_new_donors_location(self, df):
        ...
        map = folium.Map(location=[map_df[(map_df['구군명'] == selected_gugun) & (map_df['구분'] == '푸드마켓')]['위도'], 
                                   ...
                                   zoom_start=13, width=950, height=398)
    ...
    def grouping_gugun_graph20(self, gu):
        ...
        # Malgun Gothic 폰트 로드
        font_path = 'GmarketSansMedium.otf'  # Malgun Gothic 폰트 파일의 경로로 변경
...
with tab1:
    ...
    if st.button('이용자 목록 확인'):
        ...
        st.write(f'**당일 총 이용자 : {per_cnt} 명**')
        st.write(f'**총 이용 건 수 : {cnt} 건**')
...
with tab3:
    ...
    col015, col016, col017= st.columns([1.4, 1, 0.7])
    with col015:
